...
namespace LTN.CS.SCMForm.PM
{
    public partial class TruckMeasureForm_New : CoreForm, IBaseOperate
    {
        ...
        public bool isNeedTips = false;
        public bool isNeedInstrumentTip = false;
        //皮重均值提醒标识
        ...
        private void btn_Requery_Click(object sender, EventArgs e)
        {
            ...
            */
            ...
            form3.BackColor = Color.OrangeRed;
            ...
            lab.Width = 400;
            lab.Height = 150;
            lab.Text = "当前皮重与历史均值相差超过500KG";
            lab.TextAlign = ContentAlignment.MiddleCenter;
            lab.Font = new System.Drawing.Font("Tahoma", 11F, System.Drawing.FontStyle.Bold, System.Drawing.GraphicsUnit.Point, ((byte)(0)));
            ...
            btn.Click += new System.EventHandler(this.btn_Form3_Click);
            form3.ShowDialog();
            isExceed = 1;
                                    ...
                                    synchronizationContext.Send(a =>
                                    {
                                        ...
                                        {                                            
                                            if (PondDataBufferLocal.PondHardInfoForSite.MeterOneWeight.ToString() != null)
                                            {
                                                ...
                                                {
                                                    if (PondDataBufferLocal.PondHardInfoForSite.MeterOneWeight == 0 && PondDataBufferLocal.PondHardInfoForSite.MeterOneStatus.IntValue == (int)DeviceStatus.BreakDown && isNeedInstrumentTip)
                                                    {
                                                        SpeakTxtLog("仪表故障，过磅请注意！！！");
                                                        MessageBox.Show("仪表故障，过磅请注意！！！","现场故障提示",MessageBoxButtons.OK,MessageBoxIcon.Warning);
                                                        isNeedInstrumentTip = false;
                                                    }                                                    
                                                }
                                                
                                                //重量不稳定
                                                if (!string.IsNullOrEmpty(lblWeight.Text) && isNeedTips && taskTruckData != null)
                                                {
                                                    ...
                                                    {
                                                        SpeakTxtLog("过磅重量不稳定");
                                                        MessageBox.Show("该车过磅重量不稳定，请注意","现场故障提示",MessageBoxButtons.OK,MessageBoxIcon.Warning);
                                                        isNeedTips = false;
                                                    }
                        ...
                        if (dvr != null)
                        {
                            ...
                            for (int i = 0; i < pannels.Count; i++)
                            {
                                if (iChannelNum[i] > -1)
                                {
                                    ...
                                    //lpPreviewInfo.dwStreamType = (uint)(dvr.DvrSpec.Equals("B") ? 1 : 0);
                                    switch (dvr.DvrSpec)
                                    {
                                        case "B":
                                            lpPreviewInfo.dwStreamType =(uint) ((dvr.IntId == 191 && i ==5) ? 0 : 1);
                                            break;
                                        case "E":
                                            lpPreviewInfo.dwStreamType = (uint)(i < 2 ? 0 : 1);
                                            break;
                                    }
        ...
        private void ShowWeightType(BUSINESSTYPE I_BUSINESSTYPE)
        {
            ...
            //新增业务类型
            if (I_BUSINESSTYPE == BUSINESSTYPE.采购内转) radioButton5.Checked = true;
            if (I_BUSINESSTYPE == BUSINESSTYPE.火车进厂) radioButton6.Checked = true;
            if (I_BUSINESSTYPE == BUSINESSTYPE.火车集装箱) radioButton7.Checked = true;
            if (I_BUSINESSTYPE == BUSINESSTYPE.内倒_外) radioButton8.Checked = true;
        ...
        private void LogicE(bool logci = true)
        {
            try
            {
                ...
                //2023年2-3 净重不为0时无法进行退货
                if(chk_TuiHuo.Checked == true)
                {
                    if((weightNow - ptruck.N_GROSSWGT) != 0)
                    {
                        MessageBox.Show("退货任务净重不为0，需回毛重磅点称重");
                        return;
                    }
                }
                //补全皮重信息
        ...
        private void btnTareWeight_Click(object sender, EventArgs e)
        {
            lock (IsWorkingLock)
            {
                 ...
                 * */
                //进厂的 白云石粉 超过100吨  不允许过磅--考虑将该段代码注释掉
                /*
                ...
                */
                //2023-02-16修改
                if ((txt_MaterName.Text.Trim() == "白云石粉" || txt_MaterName.Text.Trim() == "石灰石粉" || txt_MaterName.Text.Trim() == "轻烧白云石块" || txt_MaterName.Text.Trim() == "生石灰块" || txt_MaterName.Text.Trim() == "石灰石|40/80" || txt_MaterName.Text.Trim() == "石灰石块40/80" || txt_MaterName.Text.Trim() == "石灰石块|40/80") && PondDataBufferLocal.PondHardInfoForSite.MeterOneWeight > 100000)
                {
                    MessageBox.Show("该物料过磅业务不能超过100吨，不允许称重，请结束任务");
                    ShowTxtLog("该物料业务不能超过100吨，不允许称重，请结束任务");
                    return;
                }
                //回皮过磅磅点限制  进厂的只能在金材2、金材3号磅点过磅        厂内因为业务广泛，难统一，皮重就没有要求

                if ((txt_MaterName.Text.Trim() == "废钢" || txt_MaterName.Text.Trim() == "生铁") && (radioButton2.Checked || radioButton4.Checked || radioButton5.Checked) && !string.IsNullOrEmpty(txt_PlanNo.Text))
                {                   
                    ...
                    if (!string.IsNullOrEmpty(plan.C_PONDLIMIT))//以物流系统说明的限制信息作为参照，该字段为空说明不需要限制
                    {
                        string[] arr = { "110", "104" };
                        if (!arr.Contains(pondSiteInfo.PondSiteNo))
                        {
                            MessageBox.Show("废钢、生铁回皮请到金材2号、3号磅过磅", "操作提示", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            //MessageBox.Show("进厂废钢、生铁回皮请到金材2号、3号磅过磅");
                            ShowTxtLog("废钢、生铁回皮请到金材2号、3号磅过磅");
                            return;
                        }
                    }                                        
                }
                ...
                IList<SM_TareWeightHistory> CarNameDatas = tareWeightService.ExecuteDB_QueryHistoryByCarName(carName);
                if (CarNameDatas.Count >= 2 && CarNameDatas != null)
                {
                    ...
                    //22.9.9 修改提示框 潘鹏
                    if ((Math.Abs(avr - PondDataBufferLocal.PondHardInfoForSite.MeterOneWeight)) > 500)
                    {
                        SpeakTxtLog("该车皮重与历史记录均值差值大于500千克");
                        //MessageBox.Show("该车皮重与历史记录均值差值大于500千克");
                        form3.StartPosition = FormStartPosition.CenterScreen;
                    ...
                    else if((Math.Abs(avr - PondDataBufferLocal.PondHardInfoForSite.MeterOneWeight))>300)
                    {
                        SpeakTxtLog("该车皮重与历史记录均值差值大于300千克");
                        form2.StartPosition = FormStartPosition.CenterScreen;
                        form2.Width = 400;
                        form2.Height = 200;

                        form2.BackColor = Color.RoyalBlue;
                        ...
                        btn.BackColor = Color.White;
                        // btn.Location = new Point(80,80);
                        btn.Click += new System.EventHandler(this.btn_Form2_Click);
                        form2.ShowDialog();
                        isExceed = 1;
                    }
                }
        ...
        private void btnGrossWeight_Click(object sender, EventArgs e)
        {
            lock (IsWorkingLock)
            {
                ...
                */
                 ...
                 * */
                //22.7.25 磅点过磅限制  潘鹏  为防止其他系统不规范发送的字段信息，先暂时加上处理
                PT_TruckMeasurePlan measurePlan = truckMeasureService.ExecuteDB_QueryTruckMeasurePlanByPlanNo_New(txt_PlanNo.Text);
                if(measurePlan == null)
                {
                    measurePlan = truckMeasureService.ExecuteDB_QueryTruckMeasurePlanByPlanNo(txt_PlanNo.Text);
                }
                if(measurePlan.C_PONDLIMIT != null && !string.IsNullOrEmpty(measurePlan.C_PONDLIMIT)&& measurePlan.C_PONDLIMIT != "0" && txt_MaterName.Text != null)
                {
                    string pondLimit = measurePlan.C_PONDLIMIT;
                    string materialName = txt_MaterName.Text.Trim();
                    string[] arr = pondLimit.Split(',');
                    if((radioButton2.Checked || radioButton4.Checked || radioButton5.Checked) && (materialName == "生铁" || materialName == "废钢"))
                    {
                        string[] arr2 = { "101", "102" };
                        if (!arr2.Contains(pondSiteInfo.PondSiteNo))
                        {
                            ...
                            MessageBox.Show(typeStr+"废钢、生铁请到金材1号南、北磅过磅", "操作提示", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            //MessageBox.Show("废钢、生铁请到金材1号南、北磅过磅!");
                            ShowTxtLog(typeStr+"废钢、生铁请到金材1号南、北磅过磅!");
                            return;
                        }
                    }
                    else
                    {
                        //内转业务分  内倒、内倒外、采购内转  这里不进行区分
                        if (!arr.Contains(pondSiteInfo.PondSiteNo))
                        {
                            MessageBox.Show("该车过磅存在磅点限制，不能在此处过磅!");
                            ShowTxtLog("该车过磅存在磅点限制，不能在此处过磅!");
                            return;
                        }
                    }
                ...
                */
                ...
                //2023-5-7  记录一车多委托的情况                
                Hashtable ht = new Hashtable();
                ht.Add("CarNo", carName);
                IList<PT_TruckMeasurePlan> planList = truckMeasureService.ExecuteDB_QueryTruckMeasureUsingPlan(ht);
                if(planList != null && planList.Count > 0)
                {
                    if(planList.Count > 1)
                    {
                        string plans = "";
                        foreach(PT_TruckMeasurePlan plan in planList)
                        {
                            plans += plan.C_PLANNO + ";";
                        }
                        string strLog = carName + "\t" + planList.Count + "\t" + plans;
                        WriteLog(strLog, "多委托记录");                        
                        ht.Add("carCount", planList.Count);
                        ht.Add("carPlans", plans);
                        ht.Add("time", DateTime.Now.ToString("yyyyMMddHHmmss"));
                        truckMeasureService.ExecuteDB_InsertTruckMultiPlansInfo(ht);
                    }
                }

                if (txt_MaterName.Text.Trim() == "煤焦油" || txt_MaterName.Text.Trim() == "粗苯" || txt_MaterName.Text.Trim() == "硫磺" || txt_MaterName.Text.Trim() == "焦化浓氨水" || txt_MaterName.Text.Trim() == "焦化净氨水" || txt_MaterName.Text.Trim() == "浓氨水")
                {
        ...
        private void WriteLog(string strLog, string strFileName)
        {
            string sFilePath = @"D:\计量系统日志写入" + "\\" + DateTime.Now.ToString("yyyyMM");
            string sFileName = strFileName + DateTime.Now.ToString("yyyy-MM-dd") + ".log";
            //文件的绝对路径
