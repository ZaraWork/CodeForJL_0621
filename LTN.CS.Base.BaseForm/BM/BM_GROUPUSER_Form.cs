...
namespace LTN.CS.Base.BaseForm.BM
{
    public partial class BM_GROUPUSER_Form : CoreForm
    {
        ...
        /// <param name="e"></param>
        private void BM_GROUPUSER_Form_Load(object sender, EventArgs e)
        {
            SetGridData(true);
        }
        /// <summary>
        ...
        private void SetLeftGridData()
        {
            WaitCallback wc = (o) =>
            {
                Action ac = () =>
                {
                    gridControl1.DataSource = userService.ExecuteDB_QueryInGroup(selectId);
        ...
        private void SetRighrGridData()
        {
            WaitCallback wc = (o) =>
            {
                Action ac = () =>
                {
                    gridControl2.DataSource = userService.ExecuteDB_QueryNotInGroup(selectId);
        ...
        private void CustomDelete()
        {
            try
            {
                if (gridView1 != null && gridView1.GetFocusedRow() != null)
                {
                    ...
                    BM_USER service = gridView1.GetFocusedRow() as BM_USER;
                    BM_GROUP_USER serviceGroup = new BM_GROUP_USER()
                    { 
                        GroupId = selectId,
                        UserId = service.IntId 
                    };
                    var rs = groupUserService.ExecuteDB_DeleteGroupUser(serviceGroup);
        ...
        private void CustomAdd()
        {
            try
            {
                if (gridView2 != null && gridView2.GetFocusedRow() != null)
                {
                    ...
                    BM_USER service = gridView2.GetFocusedRow() as BM_USER;
                    BM_GROUP_USER serviceGroup = new BM_GROUP_USER()
                    {
                        GroupId = selectId,
                        UserId = service.IntId,
                    ...
                    var rs = groupUserService.ExecuteDB_InserGroupUser(serviceGroup);
                    if (rs is CustomDBError)
                    {
                        MessageDxUtil.ShowError(((CustomDBError)rs).ErrorMsg);
                        return;
