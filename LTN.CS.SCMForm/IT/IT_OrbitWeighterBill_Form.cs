...
{
    public partial class IT_OrbitWeighterBill_Form : CoreForm
    {
        public IIT_OrbitWeighterBillService Mainservice { get; set; }
        public IT_OrbitWeighterBill_Form()
        {
            InitializeComponent();
        }
        ...
        private void Query()
        {
            ...
            {
                condition.Add("mat_no", txt_matNo.Text);
            }            
            gcl_main.DataSource = Mainservice.ExecuteDB_QueryByMatNoAndTime(condition);  
            

        }
        private void IT_OrbitWeighterBill_Form_Shown(object sender, EventArgs e)
        {
        ...
        {
            if (e.Column.FieldName == "T_WEIGHT_TIME")
            {
                if (e.Value != null)
                {
