...
namespace LTN.CS.SCMService.CS.Implement
{
    public class SM_GczTare_InfoServiceImpl : ISM_GczTare_InfoService
    {
        ...
        public IList<SM_GczTare_Info> ExecuteDB_QueryGczTareForCZ(Hashtable ht)
        {
            IList<SM_GczTare_Info> rs;
            try
            {
                rs = CommonDao.ExecuteQueryForList<SM_GczTare_Info>("selectSM_GczTare_InfoForCZByHt", ht);
            }
            catch (Exception ex)
            {
                log.Error(ex.Message);
