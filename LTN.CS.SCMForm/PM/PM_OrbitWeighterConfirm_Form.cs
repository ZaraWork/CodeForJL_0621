...
{
    public partial class PM_OrbitWeighterConfirm_Form : CoreForm
    {
        public IPM_OrbitWeighterConfirmService MainService { get; set; }
        public PM_OrbitWeighterConfirm_Form()
        {
            InitializeComponent();
        }
        ...
        private void Query()
        {
            ...
            if (!string.IsNullOrEmpty(txt_CalibrateNo.Text))
            {
                condition.Add("CalibrateNo", txt_CalibrateNo.Text);
            }
            gcl_main.DataSource = MainService.ExecuteDB_QueryOrbitWeighterBillByHashTable(condition);
        }

        private void PM_OrbitWeighterConfirm_Form_Shown(object sender, EventArgs e)
        {
